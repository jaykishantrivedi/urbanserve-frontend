      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              {["Service Name", "Category", "Description", "Status", "Actions"].map((col) => (
                <th
                  key={col}
          ...
          <tbody className="divide-y divide-gray-50">
            {isLoading ? (
              ...
              services.map((service) => (
                <tr key={service._id} className={`hover:bg-gray-50 transition-colors ${isFetching ? "opacity-60" : ""}`}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm font-semibold text-indigo-600">{service.serviceName}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 max-w-xs">
                    <span className="text-sm text-gray-600 line-clamp-2">{service.description || "-"}</span>
                  </td>
